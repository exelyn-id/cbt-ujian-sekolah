...
function doPost(e) {
  ...
  try {
    ...
    // 10. Extract Entire Database for Vercel Primary Operational DB
    else if (action === "get_all_database") {
      output = _getAllDatabase();
    }
    // 11. Save User (Admin / Teacher creation)
    else if (action === "save_user") {
      output = _saveUser(payload.user || payload);
    }
    // 12. Delete User
    else if (action === "delete_user") {
      output = _deleteUser(payload.username, payload.userId);
    }
    // 13. Save Questions Bulk
    else if (action === "save_questions") {
      output = _saveQuestionsBulk(payload.examId, payload.questionsList || payload.questions || []);
    }
    // 14. Get Questions
...
function _getAllDatabase() {
  ...
  for (var i = 0; i < rawQ.length; i++) {
    ...
    if (q.examId && q.status !== "ARCHIVED") {
      ...
      try {
        if (q.type === "MCQ_COMPLEX" || (typeof q.correctAnswer === "string" && q.correctAnswer.startsWith("{"))) {
          parsedAnswer = JSON.parse(q.correctAnswer);
        }
      } catch (e) {}

      questionsByExam[q.examId].push({
        id: q.questionId,
        orderNo: Number(q.orderNo || 1),
        type: q.type,
  ...
  var cleanUsers = users.map(function(u) {
    ...
    var role = (uName === "admin" || String(u.role || "").toUpperCase() === "ADMIN" || String(u.userId || "").startsWith("ADM")) ? "ADMIN" : "TEACHER";
    return {
      userId: u.userId || u.id || ("USR_" + Math.floor(Math.random() * 90000 + 10000)),
      username: uName,
      password: String(u.password || "").trim(),
      name: u.fullName || u.name || u["Nama Lengkap"] || uName,
      role: role,
  ...
  return _response(true, {
    users: cleanUsers,
    exams: exams,
    questions: questionsByExam
  }, "Database berhasil diekstrak.");
}

function _saveUser(user) {
  if (!user || !user.username) {
    return _response(false, null, "Username pengguna wajib diisi.");
  }

  ensureDatabaseSchema();
  var sheet = _getSheet(CONFIG.SHEETS.USERS);
  ...
  for (var r = 1; r < data.length; r++) {
    if (String(data[r][1] || "").trim().toLowerCase() === uName) {
      targetRow = r + 1;
      break;
    }
  }

  var role = user.role ? String(user.role).toUpperCase() : (uName === "admin" ? "ADMIN" : "TEACHER");
  var userId = user.userId || user.id || ((role === "ADMIN" ? "ADM_" : "TCH_") + Math.floor(Math.random() * 90000 + 10000));
  var status = user.status || "ACTIVE";
  var fullName = user.name || user.fullName || user["Nama Lengkap"] || user.username;
  var password = user.password || "123456";
  ...
  if (targetRow !== -1) {
    ...
    sheet.appendRow([userId, uName, password, fullName, status, now, now]);
  }
  ...
  return _response(true, {
    ...
    role: role,
    status: status
  }, "Pengguna berhasil disimpan ke Spreadsheet.");
}

function _deleteUser(username, userId) {
  ensureDatabaseSchema();
  var sheet = _getSheet(CONFIG.SHEETS.USERS);
  var data = sheet.getDataRange().getValues();
  ...
  for (var r = 1; r < data.length; r++) {
    var rowU = String(data[r][1] || "").trim().toLowerCase();
    var rowId = String(data[r][0] || "").trim();
    if ((uName && rowU === uName) || (userId && rowId === userId)) {
      sheet.deleteRow(r + 1);
      return _response(true, null, "Pengguna berhasil dihapus dari Spreadsheet.");
    }
  }
  return _response(false, null, "Pengguna tidak ditemukan.");
}

function _saveQuestionsBulk(examId, questionsList) {
  if (!examId || !Array.isArray(questionsList)) {
    return _response(false, null, "Data soal tidak valid.");
  }
  ensureDatabaseSchema();
  ...
  if (questionsList.length > 0) {
    ...
    for (var i = 0; i < questionsList.length; i++) {
      ...
      var qId = q.id || q.questionId || ("Q_" + Math.floor(Math.random() * 90000 + 10000));
      var ansStr = (typeof q.correctAnswer === "object") ? JSON.stringify(q.correctAnswer) : String(q.correctAnswer || "");
      var optJson = JSON.stringify(q.options || []);
  ...
  return _response(true, { count: questionsList.length }, "Soal berhasil disimpan ke Spreadsheet.");
}
