            ...
            cardsHtml = questions.map((q, qIndex) => {
                    ...
                    optionsHtml = q.options.map((opt, optIndex) => `
                        <div class="option-builder-row">
                            <div class="flex items-center gap-2" style="flex-grow: 1; min-width: 200px;">
                                ...
                                <input type="text" class="input-control opt-text-input" style="padding:0.4rem; flex-grow: 1;" value="${escapeHtml(opt.text)}" oninput="updateOptionText(${qIndex}, ${optIndex}, this.value)" ${q.type === 'TRUE_FALSE' ? 'readonly' : ''} placeholder="Teks opsi ${opt.id}">
                            </div>
                            ${q.type !== 'TRUE_FALSE' ? `
                                <div class="opt-extra">
                ...
                return `
                    <div class="card mb-4 question-card-item">
                            ...
                            <div>
                                ...
                                ${q.type === 'MCQ_COMPLEX' ? `
                                    ...
                                    <select class="input-control text-sm" onchange="updateQuestionField(${qIndex}, 'scoringMethod', this.value)">
                                        <option value="EXACT" ${q.scoringMethod === 'EXACT' ? 'selected' : ''}>Exact Set</option>
                                        <option value="PARTIAL" ${q.scoringMethod === 'PARTIAL' ? 'selected' : ''}>Partial Credit</option>
                                    </select>
                                ` : ''}
                            </div>
                        </div>
                        ...
                        <div class="mb-2">
                            <label class="input-label text-xs mb-2 block">Pilihan Jawaban & Kunci (Pilih/Centang jawaban yang benar)</label>
                            ${optionsHtml}
    ...
    window.downloadTemplateExcel = function() {
        ...
        try {
            ...
            const sampleData = [
                ...
                [2, "TRUE_FALSE", "Matahari terbit dari arah timur dan tenggelam di arah barat.", "", "Benar", "Salah", "", "", "", "Benar", 10, "Secara visual dari bumi, rotasi bumi menyebabkan matahari tampak terbit dari timur."],
        ...
        reader.onload = function(evt) {
            try {
                ...
                dataRows.forEach((row, idx) => {
                    ...
                    if (type === 'TRUE_FALSE') {
                        options = [
                            { id: 'TRUE', text: optA || 'Benar', imageUrl: '' },
                            { id: 'FALSE', text: optB || 'Salah', imageUrl: '' }
                        ];
                        const kUp = rawKey.toUpperCase();
                        correctAnswer = (kUp === 'SALAH' || kUp === 'FALSE' || kUp === 'B') ? 'FALSE' : 'TRUE';
